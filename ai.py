...
def rewrite_news():
    ...
    for item in range(0,len(src),3):
        try:
            # text = item['text']
            # print(text)
            format_of_response = f'''Скороти та перевразуй цю новину до 4-5 речень і до 1000 символів, добавляй смайли(наприклад кольори команд ✍ або такий для джерела повідомлення), але небагато. Результат виведи в цьому форматі:
                                ===Новина===
                                (тут текст 1)
                                ===Новина==='
                                (тут текст 2)
